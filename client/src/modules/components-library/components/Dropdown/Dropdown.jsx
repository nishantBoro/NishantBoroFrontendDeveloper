...
function Dropdown(props) {
  const { selectedText, items, handleItemClick, className } = props;
  const { text: selectedTextVal, id } = selectedText;
  const text =
    selectedTextVal !== ''
      ? capitalizeFirstLetter({ string: selectedTextVal })
      : 'Select';
          ...
          {Object.keys(items).map((key) => {
            ...
            return (
              <DropdownItem
                key={key}
                id={key}
                handleItemClick={handleItemSelectClick.bind(null, {
                  key,
                  text
                })}
                text={capitalizeFirstLetter({ string: text })}
