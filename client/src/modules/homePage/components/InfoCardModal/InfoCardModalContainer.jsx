...
import {
  ...
  selectSearchResultMetaData
} from '../../slices/pageData';
...
function InfoCardModalContainer() {
  ...
  const {
    infoModal: { isOpen, selectedId }
  } = useSelector((state) => selectSearchResultMetaData(state));
  const searchResData =
    useSelector((state) => selectSearchResultData(state)) || {};
  const {
    status,
    type,
